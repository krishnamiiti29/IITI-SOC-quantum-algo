...
# ---------------------------------------------------------------------------
# New, additive exact-match oracle. Does NOT modify or replace anything
# above -- ApplyHammingWeightPopcountOracle and
# build_fixed_point_grover_circuit_popcount are left exactly as they were.
#
# WHY THIS EXISTS: HW(x) XOR HW(y) == target_parity is satisfied by roughly
# HALF of all valid (x, y) states (parity only has two possible values), not
# just the one specific chosen key. That means
# ApplyHammingWeightPopcountOracle marks an entire equivalence class of
# states identically -- Grover concentrates amplitude on that whole class,
# but has no way to prefer the true key over any of its ~30 parity-siblings
# within it. This oracle instead marks ONLY the exact (key_x, key_y) state
# -- a unique condition -- so Grover actually converges on the real target.
# Checking bit-for-bit equality is strictly stronger than checking Hamming-
# weight parity (x == key_x already implies HW(x) == HW(key_x)), so no
# popcount ancillas are needed for this version; it operates directly on
# the search register, exactly like a textbook single-item Grover oracle.
# ---------------------------------------------------------------------------

def ApplyExactKeyOracle(Space, width_qubits, height_qubits, key, angle=PI3_ANGLE):
    """
    Marks only the single basis state |key_x, key_y> with the shared pi/3
    phase (same select -> phase -> unselect pattern used elsewhere in this
    file), leaving every other state untouched. Because exactly one state
    satisfies this condition, Grover's amplitude has a genuine unique
    ...
    # Select |key_x, key_y>: flip qubits that should read 0 for this key.
    for i, q in enumerate(x_qubits):
        if ((key_x >> i) & 1) == 0:
            Space.x(q)
            flipped.append(q)
    ...
    # pi/3 phase, applied only when every search qubit now reads 1 --
    # i.e. only for the original |key_x, key_y> state.
    if len(controls) == 1:
        Space.p(angle, controls[0])
    else:
        gate = MCPhaseGate(angle, num_ctrl_qubits=len(controls) - 1)
        Space.append(gate, controls)
...
def build_fixed_point_grover_circuit_exact_key(Space, width_qubits, height_qubits, key,
                                                iterations, angle=PI3_ANGLE):
    """
    New, additive Grover assembly using ApplyExactKeyOracle: `iterations`
    repetitions of (mark only the true key) + (matched pi/3 diffuser). This
    is the version that should actually converge measurement probability
    onto the real chosen key, unlike the parity-based oracles above.
    """
    n = width_qubits + height_qubits
    diffuser = build_pi3_diffuser(n, angle)

    for _ in range(iterations):
        Space = ApplyExactKeyOracle(Space, width_qubits, height_qubits, key, angle)
        Space.append(diffuser, list(range(n)))
